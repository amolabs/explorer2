...
/**
 * @swagger
 * /chain/{chain_id}/accounts/{address}/parcels:
 *   parameters:
 *     - $ref: '#/definitions/ChainId'
 *     - $ref: '#/definitions/Address'
 *     - name: top
 ...
 *     tags:
 *       - accounts
 *     description: Get parcel list by owner address
 *     produces:
 *       - application/json
 *     responses:
 *       200:
 ...
 */
router.get('/:address([a-fA-F0-9]+)/parcels', function(req, res) {
  const chain_id = res.locals.chain_id;
  var top = req.query.top || 0;
  var from = req.query.from || 0;
  var num = req.query.num || 20;
  parcel.getListByOwner(chain_id, req.params.address, top, from, num)
    .then((rows) => {
      res.status(200);
      res.send(rows);
    })
