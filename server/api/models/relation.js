    ...
    if (include_tx) {
      query_str += `\
        ( \
          ...
          FROM \`${dbs['builder']}\`.\`r_account_tx\` rat \
            LEFT JOIN \`${dbs['collector']}\`.\`c_txs\` ON rat.chain_id = c_txs.chain_id \
      ...
      `;
      if (anchor > 0) {
        query_var = query_var.concat([chain_id, address, anchor, from + num]);
      } else {
        // anchor height will not be used when anchor = 0
        query_var = query_var.concat([chain_id, address, from + num]);
      }
    }
    if (include_block) {
      ...
      query_str += `\
        ( \
          SELECT rab.chain_id, rab.address, \
            ...
            '' tx_fee, '' tx_payload \
          FROM \`${dbs['builder']}\`.\`r_account_block\` rab \
          WHERE rab.chain_id = ? \
            AND rab.address = ? `
...
async function getParcelHistory(chain_id, parcel_id, top, from, num) {
  return new Promise(function(resolve, reject) {
    ...
    query_str = `\
      SELECT c_txs.* \
      FROM \`${dbs['builder']}\`.\`r_parcel_tx\` rpt \
        LEFT JOIN \`${dbs['collector']}\`.\`c_txs\` ON rpt.chain_id = c_txs.chain_id \
