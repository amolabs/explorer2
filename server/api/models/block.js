...
async function getOne(chain_id, height) {
  return new Promise(function(resolve, reject) {
    ...
    var query_var;
    if (height == 0) {
      query_str = `select * from \`${dbs['collector']}\`.c_blocks where (chain_id = ?) \
...
async function getLast(chain_id) {
  return new Promise(function(resolve, reject) {
    var query_str = `SELECT * FROM \`${dbs['collector']}\`.c_blocks WHERE (chain_id = ?) \
      ORDER BY height DESC LIMIT 1`;
...
async function getList(chain_id, anchor, from, num) {
  return new Promise(function(resolve, reject) {
    ...
    var query_var;
    if (anchor == 0) {
      query_str = `select * from \`${dbs['collector']}\`.\`c_blocks\` \
        where (\`chain_id\` = ?) \
