    ...
    var query_str = `SELECT \
      ...
      FROM ( \
        SELECT \
          ...
          FROM \`${dbs['collector']}\`.\`c_blocks\` b \
            LEFT JOIN \`${dbs['collector']}\`.\`c_txs\` t \
            ON b.height = t.height AND b.chain_id = t.chain_id  \
...
async function getTxStat(chain_id, num_txs) {
  return new Promise(function(resolve, reject) {
    ...
    var query_str = `SELECT \
      ...
      FROM ( \
        SELECT * FROM \`${dbs['collector']}\`.\`c_txs\` \
        WHERE chain_id = ? \
...
async function getAssetStat(chain_id) {
  return new Promise(function(resolve, reject) {
    var query_str = `select * from \`${dbs['builder']}\`.\`asset_stat\` \
    where (chain_id = ?) limit 1`;
  ...
  return new Promise(function(resolve, reject) {
    ...
    db.query(query_str, query_var, function (err, rows, fields) {
      ...
      query_str = `SELECT IFNULL(AVG(t.amount), 0) AS avg \
        FROM ( \
          SELECT amount FROM \`${dbs['builder']}\`.\`s_incentives\` WHERE chain_id = ? \
...
async function getDraftStat(chain_id) {
  return new Promise(function(resolve, reject) {
    ...
    var query_str = `SELECT COUNT(*) num_drafts, \
      ...
      AS num_passed \
      FROM \`${dbs['builder']}\`.\`s_drafts\` WHERE (chain_id = ?)`;
    var query_var = [chain_id];
    db.query(query_str, query_var, function (err, rows, fields) {
      if (err) {
        return reject(err);
...
async function getStorageStat(chain_id) {
  return new Promise(function(resolve, reject) {
    var query_str = `SELECT count(*) num_storages FROM \`${dbs['builder']}\`.\`s_storages\` \
      WHERE (chain_id = ?)`;
    var query_var = [chain_id];
...
async function getNodeStat(chain_id) {
  return new Promise(function(resolve, reject) {
    var query_str = `SELECT count(*) num_nodes FROM \`${dbs['nodes']}\`.\`nodes\` \
      WHERE (chain_id = ?)`;
    var query_var = [chain_id];
    db.query(query_str, query_var, function (err, rows) {
      if (err) {
