...
def collect_block(s, node, height):
    ...
    incs = collect_incentive(s, node, height)
...
def collect_block_txs(s, node, height):
    # collect txs
    r = s.get(f'{node}/tx_search?query="tx.height={height}"')
